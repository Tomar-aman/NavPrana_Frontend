"use client";

import { useEffect, useRef, useCallback } from "react";
import { useDispatch } from "react-redux";
import { useRouter } from "next/navigation";
import { toast } from "sonner";
import { Smartphone } from "lucide-react";
import { googleLogin } from "@/redux/features/authSlice";

const GOOGLE_CLIENT_ID = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;

/**
 * The "or continue with" row. Pass onPhoneClick to sit a phone sign-in button
 * beside Google; without it the row is Google alone, as it was before.
 */
const GoogleSignInButton = ({ onPhoneClick }) => {
  const dispatch = useDispatch();
  const router = useRouter();
  const tokenClientRef = useRef(null);

  const handleGoogleSuccess = useCallback(
    async (tokenResponse) => {
      try {
        await dispatch(googleLogin(tokenResponse.access_token)).unwrap();
        toast.success("Login successful");
        router.push("/");
      } catch (err) {
        toast.error(err || "Google login failed");
      }
    },
    [dispatch, router],
  );

  useEffect(() => {
    const existingScript = document.getElementById("google-gsi-script");

    const initializeGoogle = () => {
      if (!window.google?.accounts?.oauth2) return;

      tokenClientRef.current =
        window.google.accounts.oauth2.initTokenClient({
          client_id: GOOGLE_CLIENT_ID,
          scope: "email profile",
          callback: handleGoogleSuccess,
        });
    };

    if (existingScript) {
      initializeGoogle();
    } else {
      const script = document.createElement("script");
      script.id = "google-gsi-script";
      script.src = "https://accounts.google.com/gsi/client";
      script.async = true;
      script.defer = true;
      script.onload = initializeGoogle;
      document.head.appendChild(script);
    }
  }, [handleGoogleSuccess]);

  const handleClick = () => {
    if (tokenClientRef.current) {
      tokenClientRef.current.requestAccessToken();
    }
  };

  return (
    <div className="w-full">
      {/* Divider */}
      <div className="flex items-center gap-3 my-4">
        <div className="flex-1 h-px bg-gray-200" />
        <span className="text-xs text-muted-foreground font-medium">or continue with</span>
        <div className="flex-1 h-px bg-gray-200" />
      </div>

      {/* Google button — paired with phone sign-in wherever that is offered */}
      <div className={onPhoneClick ? "grid grid-cols-2 gap-3" : ""}>
        <button
          type="button"
          onClick={handleClick}
          className="w-full flex items-center justify-center gap-3 py-3 px-4
            border border-gray-200 rounded-xl bg-white text-foreground
            hover:bg-gray-50 hover:border-gray-300 transition-all text-sm font-medium cursor-pointer"
        >
          <svg viewBox="0 0 24 24" width="18" height="18">
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
          </svg>
          Google
        </button>

        {onPhoneClick && (
          <button
            type="button"
            onClick={onPhoneClick}
            className="w-full flex items-center justify-center gap-3 py-3 px-4
              border border-gray-200 rounded-xl bg-white text-foreground
              hover:bg-gray-50 hover:border-gray-300 transition-all text-sm font-medium cursor-pointer"
          >
            <Smartphone size={18} className="text-primary" />
            Phone
          </button>
        )}
      </div>
    </div>
  );
};

export default GoogleSignInButton;
